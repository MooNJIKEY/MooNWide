...
public class ChatUI extends Widget {
    ...
    public ChatUI() {
	this(0, UI.scale(Math.max(minh, Utils.getprefi("chatsize", minh))), false);
    }
    ...
    public ChatUI(int w, int h, boolean windowed) {
	super(Coord.of(Math.max(0, w), Math.max(UI.scale(minh), h)));
	...
	public static abstract class Message {
	    ...
	    protected long autoTransGenSeen = -1;

	    public abstract Indir<Text> render(int w);

	    /** When auto-translate updates, return false so {@link RenderedMessage} rebuilds RichText. */
	    protected boolean autoTranslateStillFresh() {
		if(!LocalizationManager.autoTranslateChatRefresh())
		    return(true);
		long g = LocalizationManager.autoTranslateUiGenerationSampled();
		if(autoTransGenSeen < 0) {
		    autoTransGenSeen = g;
		    return(true);
		}
		if(autoTransGenSeen != g) {
		    autoTransGenSeen = g;
		    return(false);
		}
		return(true);
		...
		return(autoTranslateStillFresh());
	    }
	...
	public static class SimpleMessage extends Message {
	    ...
	    private long renderGen = Long.MIN_VALUE;
	    ...
	    private String translated() {
		long gen = LocalizationManager.autoTranslateUiGenerationSampled();
		if((translated == null) || (renderGen != gen)) {
		    translated = LocalizationManager.autoTranslateProcessed(text);
		    renderGen = gen;
		}
		return(translated);
	    }
		...
		String body = translated();
		MoonPerfOverlay.countTextRender();
		if(col == null)
		    return(() -> fnd.render(RichText.Parser.quote(body), w));
		else
		    return(() -> fnd.render(RichText.Parser.quote(body), w, TextAttribute.FOREGROUND, col));
	    }
	}
	...
	private static boolean isMineSupportUpdateNotice(String line) {
	    if(line == null)
		return(false);
	    String s = line.toLowerCase(java.util.Locale.ROOT);
	    if(s.contains("please update your client")
	       && s.contains("coverage")
	       && (s.contains("mine-support") || s.contains("mine support")))
		return(true);
	    return(s.contains("restore mine-support coverage display")
		|| s.contains("restore mine support coverage display"));
	}
	...
	public String name() {return(MoonL10n.chatChan(this.name));}

	@Override
	public void uimsg(String msg, Object... args) {
	    if((msg == "msg") || (msg == "log")) {
		String line = (String)args[0];
		if(isMineSupportUpdateNotice(line))
		    return;
		if(Utils.getprefb("moon-suppress-minesupport-msg", false)
		    && line != null
		    && (line.toLowerCase(java.util.Locale.ROOT).contains("mine-support")
			|| line.toLowerCase(java.util.Locale.ROOT).contains("mine support")))
		    return;
		Color col = null;
		if(args.length > 1)
	...
	public class NamedMessage extends Message {
		...
		public Text get() {
		    String body = translated();
		    MoonPerfOverlay.countTextRender();
		    return(fnd.render(RichText.Parser.quote(String.format("%s: %s", nm, body)), w, TextAttribute.FOREGROUND, col));
		}
	    }
	...
	private void mutemenu(int pl, boolean cur) {
	    SListMenu.Action ma = SListMenu.Action.of(cur ? LocalizationManager.tr("chat.unmute") : LocalizationManager.tr("chat.mute"), () -> wdgmsg("mute", pl, cur ? 0 : 1));
	...
	public String name() {
	    return(MoonL10n.chatChan("Party"));
	}
	...
	private class DarkChannel {
	    ...
	    public Text rname() {
		String name = MoonL10n.chatChan(chan.name());
	...
	private Notification(Channel chan, Channel.Message msg) {
	    this.chan = chan;
	    this.msg = msg;
	    this.chnm = fnd.render(MoonL10n.chatChan(chan.name()), 0, TextAttribute.FOREGROUND, Color.WHITE);
	...
	if(qline != null) {
		...
		String pre = String.format("%s> ", MoonL10n.chatChan(qline.chan.name()));
