...
public class UI {
    ...
    private double moonPlayDedupeUntil;
    private String moonPlayDedupeKey;
    ...
    private class WidgetConsole extends Console {
	{
	    ...
	    setcmd("atstat", new Command() {
		    public void run(Console cons, String[] args) {
			cons.out.println(LocalizationManager.autoTranslateMetricsLine());
		    }
		});
	...
	public void run() {
	    ...
	    if("sess".equals(typenm))
		MoonConnectLog.log("ui: NewWidget running type=sess id=" + id);
	    if(wdg == null)
	...
	if(id < 0) {
	    if("play".equals(msg))
		MoonConnectLog.log("client: Play not sent - widget not bound to UI id (sender=" + sender.getClass().getSimpleName() + ")");
	    new Warning("wdgmsg sender (%s) is not in rwidgets, message is %s", sender.getClass().getName(), msg).issue();
	    return;
	}
	if(rcvr != null) {
	    Object[] out = MoonPacketHook.fireOutgoing(this, id, msg, args);
	    if(out == null) {
		if("play".equals(msg))
		    MoonConnectLog.log("client: Play not sent - outgoing filter dropped wdgmsg");
		return;
	    }
	    if("play".equals(msg) && out.length > 0) {
		String dedupeKey = id + "\0" + String.valueOf(out[0]);
		double now = Utils.rtime();
		if(dedupeKey.equals(moonPlayDedupeKey) && now < moonPlayDedupeUntil) {
		    MoonConnectLog.log("client: wdgmsg play skipped (same char, wait "
			+ String.format(java.util.Locale.ROOT, "%.1f", moonPlayDedupeUntil - now) + "s before retry)");
		    return;
		}
		moonPlayDedupeKey = dedupeKey;
		moonPlayDedupeUntil = now + 2.0;
		String a0 = (out[0] != null) ? String.valueOf(out[0]) : "";
		MoonConnectLog.log("client: queued wdgmsg play to game server wid=" + id + " char=" + a0);
	    }
	    if(MoonConfig.debugSpeedWire && "set".equals(msg) && out.length >= 1) {
		Speedget sg = MoonPacketHook.speedgetForBoundWidget(this, id);
		if(sg != null) {
		    try {
			int v = Utils.iv(out[0]);
			System.err.println("[MoonSpeedWire] SEND set wid=" + id + " wireIdx=" + v
			    + " wdgMax=" + sg.max + " wdgCur=" + sg.cur + " (bytes queued for server)");
		    } catch(RuntimeException ignored) {}
		}
	    }
	    rcvr.rcvmsg(id, msg, out);
	}
    }
