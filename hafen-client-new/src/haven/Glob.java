...
public class Glob {
    ...
    /** {@link MapView} player gob id for this world; used by movement hooks when static id is stale. */
    public volatile long moonPlayerGobId = -1;
    ...
    /** Blended lighting for map (see {@link MoonConfig#nightVisionBlend}). */
    ...
    /** Last sampled daylight (server) for {@link MoonConfig#alwaysDaylight} override when {@link Astronomy#night}. */
    private Color cachedDayAmb=null, cachedDayDif=null, cachedDaySpc=null;
    private double cachedDayAng=0, cachedDayElev=0;
	...
	applyLocalDaylightVisual();
	moonRefreshBlight();
    }

    /** Recompute {@link #blightamb}/{@link #blightdif}/{@link #blightspc} from current {@link #lightamb} and prefs. */
    public void moonRefreshBlight() {
	synchronized(this) {
	    double bright = MoonConfig.nightVisionBlend;
	    if(lightamb != null) {
		blightamb = Utils.blendcol(lightamb, Color.WHITE, bright);
		blightdif = (lightdif != null) ? Utils.blendcol(lightdif, Color.WHITE, bright) : null;
		blightspc = (lightspc != null) ? Utils.blendcol(lightspc, Color.WHITE, bright) : null;
	    } else {
		blightamb = blightdif = blightspc = null;
	    }
	}
    }

    /** Alias for UI / light blob updates. */
    public void brighten() {
	moonRefreshBlight();
    }

    private void applyLocalDaylightVisual() {
	if(!MoonConfig.alwaysDaylight)
	    return;
	lightamb = MoonConfig.DAYLIGHT_FALLBACK_AMB;
	lightdif = MoonConfig.DAYLIGHT_FALLBACK_DIF;
	lightspc = MoonConfig.DAYLIGHT_FALLBACK_SPC;
	lightang = MoonConfig.DAYLIGHT_FALLBACK_ANG;
	lightelev = MoonConfig.DAYLIGHT_FALLBACK_ELEV;
    }
	...
	while(!msg.eom()) {
		...
		synchronized(this) {
		    ...
		    if(MoonConfig.alwaysDaylight) {
			tlightamb = MoonConfig.DAYLIGHT_FALLBACK_AMB;
			tlightdif = MoonConfig.DAYLIGHT_FALLBACK_DIF;
			tlightspc = MoonConfig.DAYLIGHT_FALLBACK_SPC;
			tlightang = MoonConfig.DAYLIGHT_FALLBACK_ANG;
			tlightelev = MoonConfig.DAYLIGHT_FALLBACK_ELEV;
		    }
		    if(inc) {
			olightamb = lightamb;
			olightdif = lightdif;
