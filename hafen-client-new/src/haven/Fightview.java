...
public class Fightview extends MovableWidget {
    ...
    public void uimsg(String msg, Object... args) {
	    ...
	    MoonFightHud.onRelationBegin(rel.gobid, rel.ip, rel.oip);
            lsrel.addFirst(rel);
	    updrel();
            return;
        } else if(msg == "del") {
            Relation rel = getrel(Utils.uiv(args[0]));
	    MoonFightHud.onRelationRemoved(rel.gobid);
	    rel.remove();
            lsrel.remove(rel);
	    if(rel == current)
		setcur(null);
	    updrel();
            ...
            Relation rel = getrel(Utils.uiv(args[0]));
	    int newIp = Utils.iv(args[2]);
	    int newOip = Utils.iv(args[3]);
	    MoonFightHud.onRelationIpUpdate(this, rel, newIp, newOip);
	    rel.give(Utils.iv(args[1]));
	    rel.ip = newIp;
	    rel.oip = newOip;
            return;
