...
public class Config {
    ...
    /** Ask "Are you sure you want to exit?" when closing window. */
    public static boolean confirmclose = Utils.getprefb("confirmclose", false);

    /** Short names for default highlight/hide/sound items (res path -> display name). */
    public static final Map<String, String> defaultitems = new HashMap<>();

    /** Static list of saved accounts. Load with reloadLogins(), persist with saveLogins(). */
    public static final List<LoginData> logins = Collections.synchronizedList(new java.util.ArrayList<>());

    public static void reloadLogins() {
	logins.clear();
	logins.addAll(Config.get().loadLogins());
    }

    public static void saveLogins() {
	Config.get().saveLogins(new java.util.ArrayList<>(logins));
    }

    public java.util.List<LoginData> loadLogins() {
	java.util.List<String> raw = Utils.getprefsl("logins", new String[0]);
	java.util.List<LoginData> ret = new java.util.ArrayList<>();
	for(String s : raw) {
	    int t = s.indexOf('\t');
	    if(t >= 0)
		ret.add(new LoginData(s.substring(0, t), s.substring(t + 1)));
	}
	return(ret);
    }

    public void saveLogins(java.util.List<LoginData> logins) {
	java.util.List<String> raw = new java.util.ArrayList<>();
	for(LoginData ld : logins)
	    raw.add(ld.username + "\t" + ld.password);
	Utils.setprefsl("logins", raw);
    }
	...
	out.println("  -b                 Borderless fullscreen window");
	...
	out.println("  -J USER             Auto-login with saved account USER (second client; same prefs as main)");
    }

    /** Set from {@code -J USER}; applied after parsing to {@link Bootstrap#setPendingReconnectCred}. */
    private static String moonJoinLoginUser = null;

    public static void cmdline(String[] args) {
	PosixArgs opt = PosixArgs.getopt(args, "hdPGfbU:r:S:u:C:p:J:");
	if(opt == null) {
	    usage(System.err);
	    System.exit(1);
	}
	for(char c : opt.parsed()) {
		...
		break;
	    case 'b':
		MainFrame.initborderless.set(true);
		break;
	    case 'r':
		...
		break;
	    case 'J':
		moonJoinLoginUser = opt.arg;
		break;
	    }
	}
	if(moonJoinLoginUser != null) {
	    String want = moonJoinLoginUser;
	    moonJoinLoginUser = null;
	    reloadLogins();
	    synchronized(logins) {
		for(LoginData ld : logins) {
		    if(ld.username.equals(want)) {
