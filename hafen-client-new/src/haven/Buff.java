...
public class Buff extends Widget implements ItemInfo.ResOwner, Bufflist.Managed {
    ...
    /** Buff charge / duration meter 0..1 when present (combat AI / HUD). */
    public Double buffameter() {
	return(ameteri.get());
    }

    /** Circular meter 0..1 when present. */
    public Double buffcmeter() {
	return(cmeteri.get());
    }
	...
	try {
	    if(rawinfo != null) {
		BufferedImage img = ItemInfo.shorttip(info());
		return((img != null) ? img : Text.render("").img);
	    }
	    String ret = res.get().flayer(Resource.tooltip).t;
	    return(Text.render(LocalizationManager.autoTranslateProcessed(ret)).img);
	} catch(RuntimeException e) {
	    new Warning(e, "Failed to build short buff tooltip").issue();
	...
	try {
		...
		Resource.Pagina pag = res.get().layer(Resource.pagina);
		if(pag != null)
		    img = ItemInfo.catimgs(0, img, RichText.render("\n" + LocalizationManager.autoTranslateProcessed(pag.text), textw).img);
	    }
	    return(img);
	} catch(RuntimeException e) {
	    new Warning(e, "Failed to build long buff tooltip").issue();
	    return(Text.render("").img);
	}
    }
