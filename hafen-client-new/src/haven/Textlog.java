...
public class Textlog extends Widget {
	...
	String t = LocalizationManager.autoTranslateProcessed(line);
	Text rl;
	if(col == null)
	    rl = fnd.render(RichText.Parser.quote(t), sz.x - (margin * 2) - sflarp.sz().x);
	else
	    rl = fnd.render(RichText.Parser.quote(t), sz.x - (margin * 2) - sflarp.sz().x, TextAttribute.FOREGROUND, col);
	synchronized(lines) {
	    lines.add(rl);
	}
	if(cury == maxy)
