...
public class QuestWnd extends Widget {
	...
	public String title() {
	    if(title != null)
		return(LocalizationManager.autoTranslateProcessed(title));
	    return(LocalizationManager.autoTranslateProcessed(res.get().flayer(Resource.tooltip).t));
	}
	...
	public void done(GameUI parent) {
	    parent.add(new Widget() {
			...
			if(img == null) {
				...
				msg = (done == QST_DONE)
				    ? catf.render(LocalizationManager.tr("quest.done.banner")).tex()
				    : failf.render(LocalizationManager.tr("quest.fail.banner")).tex();
				/*
	...
	public static class DefaultCond extends CondWidget {
		...
		buf.append(String.format("%s{%c %s", RichText.Parser.col2a(stcol[cond.done]), stsym[cond.done], LocalizationManager.autoTranslateProcessed(cond.desc)));
		if(cond.status != null) {
		    buf.append(' ');
		    buf.append(LocalizationManager.autoTranslateProcessed(cond.status));
		}
		buf.append("}");
	...
	public static class QView extends Widget {
	    ...
	    public static final Text.Foundry qcfnd = new Text.Foundry(Text.sans, 13).aa(true);
		...
		String desc = LocalizationManager.autoTranslateProcessed(c.desc);
		String status = (c.status != null) ? LocalizationManager.autoTranslateProcessed(c.status) : null;
		return(qcfnd.render(" " + stsym[c.done] + " " + desc + ((status != null) ? (" " + status) : ""), stcol[c.done]));
	    }
	...
	prev = add(CharWnd.settip(new MoonLocalizedLabel("chr-hdr-quests", "quest.log", CharWnd.width, UI.scale(58)), "gfx/hud/chr/tips/quests"), new Coord(0, 0));
	questbox = add(new Widget(new Coord(attrw, height)) {
	    ...
	    }, prev.pos("bl").adds(8, 2).add(wbox.btloff()));
	...
	addhlp(lists.c.add(0, lists.sz.y + UI.scale(5)), UI.scale(5), lists.sz.x,
		     lists.new TabButton(0, LocalizationManager.tr("quest.tab.current"),   cqst),
		     lists.new TabButton(0, LocalizationManager.tr("quest.tab.done"), dqst));
	pack();
    }
