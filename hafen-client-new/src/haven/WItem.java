...
public class WItem extends Widget implements DTarget {
	...
	if(seq != moonqSeq) {
		...
		/* Do not advance moonqSeq: info may still be resolving resources with the same infoseq. */
		moonqVal = null;
	    }
	}
	return(moonqVal);
    ...
    private final MoonItemQuality.SlotCache invQualCache = new MoonItemQuality.SlotCache();

    public void clearInventoryQualityOverlayCache() {
	invQualCache.clear();
	moonqSeq = -1;
	moonqVal = null;
    }
    ...
    private void drawQuality(GOut g, Coord sz) {
	Double q = moonItemQ();
	if(q == null || q <= 0)
	    return;
	...
	if(InventoryItemClickPolicy.handle(this, ev))
	    return(true);
