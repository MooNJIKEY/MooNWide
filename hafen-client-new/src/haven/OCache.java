...
public class OCache implements Iterable<Gob> {
    ...
    /** Run on every live gob (world + local/virtual collections). */
    public void gobAction(Consumer<Gob> action) {
	synchronized(this) {
	    for(Gob g : this)
		action.accept(g);
	    for(Collection<Gob> gobs : local) {
		for(Gob g : gobs)
		    action.accept(g);
	    }
	}
	...
	private static void maybeFightFloatDamage(Gob g, Indir<Resource> res, byte[] sdt) {
	    if(g == null || res == null || sdt == null || sdt.length < 7)
		return;
	    try {
		Resource rr = res.get();
		if(rr == null || rr.name == null || !rr.name.equals("gfx/fx/floatimg"))
		    return;
		MessageBuf buf = new MessageBuf(sdt);
		int dmg = buf.int32();
		buf.uint8(); /* ignored byte used by floatimg */
		int clr = buf.uint16();
		/* Float colors: red=61455, yellow=64527, armor=36751 */
		if(clr != 61455 && clr != 64527 && clr != 36751)
		    return;
		MoonFightHud.onFloatDamage(g.id, dmg, clr, g.rc);
	...
	public void apply(Gob g, AttrDelta msg) {
	    ...
	    maybeFightFloatDamage(g, res, sdt);
	    if(res != null) {
