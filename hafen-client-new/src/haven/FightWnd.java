...
public class FightWnd extends Widget {
    ...
    public static final Text.Foundry namef = new Text.Foundry(Text.serif.deriveFont(java.awt.Font.BOLD), 18).aa(true);
	...
	public String rendertext() {
	    ...
	    if(pag != null)
		buf.append(LocalizationManager.autoTranslateProcessed(pag.text));
	    return(buf.toString());
	}
	...
	public List<ItemInfo> info() {
		...
		if(rawinfo != null)
		    ...
		    info = Arrays.asList(new ItemInfo.Name(this, LocalizationManager.autoTranslateProcessed(res.get().flayer(Resource.tooltip).t)));
	    }
	    return(info);
	}
	...
	public BufferedImage renderinfo(int width) {
	    ...
	    if(nm != null)
		l.cmp.add(namef.render(nm.nameText()).img, new Coord(0, l.cmp.sz.y + UI.scale(10)));
	...
	for(Action act : acts)
	    u += act.u;
	count.settext(String.format(LocalizationManager.tr("fight.used_fmt"), u, maxact));
	...
	public class Item extends Widget implements DTarget {
		...
		add(IconText.of(Coord.of(prev.c.x - UI.scale(2), sz.y), act::rendericon, () -> LocalizationManager.autoTranslateProcessed(act.res.get().flayer(Resource.tooltip).t)), Coord.z);
	    }
	...
	public void tick(double dt) {
		...
		for(Action act : acts) {
		    try {
			act.name = LocalizationManager.autoTranslateProcessed(act.res.get().flayer(Resource.tooltip).t);
		    } catch(Loading l) {
			act.name = "...";
			loading = true;
		    }
    ...
    private Text unused = new Text.Foundry(attrf.font.deriveFont(java.awt.Font.ITALIC)).aa(true).render(LocalizationManager.tr("fight.unused"));
	...
	add(CharWnd.settip(new MoonLocalizedLabel("chr-hdr-fight", "fight.hdr", CharWnd.width, UI.scale(58)), "gfx/hud/chr/tips/combat"), 0, 0);
	...
	count = add(new Label("", attrf), p.pos("ur").adds(10, 0));
	...
	p = add(new Button(UI.scale(110), LocalizationManager.tr("fight.load"), false).action(() -> {
		    load(savelist.sel);
		    use(savelist.sel);
	}), p.pos("ur").adds(10, 0));
	p = add(new Button(UI.scale(110), LocalizationManager.tr("fight.save"), false).action(() -> {
		    if(savelist.sel < 0) {
			getparent(GameUI.class).error(LocalizationManager.tr("fight.err.nosel"));
		    } else {
			save(savelist.sel);
			use(savelist.sel);
		    }
	...
	if(nm == "avail") {
		...
		if((fl & (1 << i)) != 0) {
			...
			saves[i] = attrf.render(LocalizationManager.autoTranslateProcessed((String)args[i + 1]));
		    else
			saves[i] = attrf.render(String.format(LocalizationManager.tr("fight.saved_fmt"), i + 1));
		} else {
		    saves[i] = unused;
		}
	    }
