...
public abstract class ItemInfo {
	...
	public BufferedImage render() {
		...
		try {
		    ...
		    new Warning(e, String.format("Ignoring broken item-info tip %s", tip.getClass().getName())).issue();
		}
	    }
	    return(cmp.compose());
	}
	...
	public BufferedImage tipimg() {
	    return(Text.render(LocalizationManager.autoTranslateProcessed(raw)).img);
	}
    }

    public static class Name extends Tip {
	public final Text str;
	/** When set, name is re-rendered each layout so async LibreTranslate can refresh tooltips. */
	public final String dynsrc;
	...
	public Name(Owner owner, String s) {
	    super(owner);
	    this.str = null;
	    this.dynsrc = s;
	}

	/** Resolved label (dynamic path includes l10n + auto-translate). */
	public String nameText() {
	    if(dynsrc != null)
		return(Text.render(LocalizationManager.autoTranslateProcessed(dynsrc)).text);
	    return((str != null) ? str.text : "");
	}

	public BufferedImage tipimg() {
	    if(dynsrc != null) {
		String t = LocalizationManager.autoTranslateProcessed(dynsrc);
		return(Text.render((t != null) ? t : "").img);
	    }
	    if(str != null)
		return(str.img);
	...
	public Tip shortvar() {
	    return(new Tip(owner) {
		    public BufferedImage tipimg() {return(Name.this.tipimg());}
		    public int order() {return(0);}
		});
	}
	...
	public BufferedImage tipimg(int w) {
	    return(RichText.render(LocalizationManager.autoTranslateProcessed(str), w).img);
	}
	...
	public BufferedImage tipimg() {
	    ...
	    g.drawImage(Text.render(LocalizationManager.tr("iteminfo.contents")).img, 0, 0, null);
