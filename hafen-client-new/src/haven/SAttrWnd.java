...
public class SAttrWnd extends Widget {
	...
	private SAttr(Glob glob, String attr, Color bg) {
	    ...
	    int textw = Math.max(UI.scale(48), sub.c.x - UI.scale(18) - (img.sz().x + UI.scale(10)));
	    this.rnm = attrf.ellipsize(LocalizationManager.autoTranslateProcessed(res.flayer(Resource.tooltip).t), textw);
	}
	...
	private StudyInfo(Coord sz, Widget study) {
	    ...
	    this.study = study;
	    Widget plbl, pval;
	    plbl = add(new Label(LocalizationManager.tr("chr.sattr.attention")), UI.scale(2, 2));
	    ...
	    plbl = add(new Label(LocalizationManager.tr("chr.sattr.exp_cost")), pval.pos("bl").adds(0, 2).xs(2));
	    ...
	    plbl = adda(new Label(LocalizationManager.tr("chr.sattr.lp")), pval.pos("ul").subs(0, 2).xs(2), 0.0, 1.0);
	}
	...
	Widget prev;
	prev = add(CharWnd.settip(new MoonLocalizedLabel("chr-hdr-abilities", "chr.sattr.abilities", CharWnd.width, UI.scale(58)), "gfx/hud/chr/tips/sattr"), Coord.z);
	attrs = new ArrayList<>();
	SAttr aw;
	attrs.add(aw = add(new SAttr(glob, "unarmed", every), prev.pos("bl").adds(8, 2).add(wbox.btloff())));
	...
	prev = add(CharWnd.settip(new MoonLocalizedLabel("chr-hdr-study", "chr.sattr.study", CharWnd.width, UI.scale(58)), "gfx/hud/chr/tips/study"), width, 0);
	studyc = prev.pos("bl").adds(8, 2);
	Widget bframe = adda(new Frame(new Coord(attrw, UI.scale(105)), true), prev.pos("bl").adds(8, 2).x, lframe.pos("br").y + UI.scale(2), 0.0, 1.0);
	int rx = bframe.pos("iur").subs(10, 0).x;
	prev = add(new Label(LocalizationManager.tr("chr.sattr.exp_pts")), bframe.pos("iul").adds(10, 5));
	adda(enclabel(), new Coord(rx, prev.pos("ul").y), 1.0, 0.0);
	prev = add(new Label(LocalizationManager.tr("chr.sattr.lp")), prev.pos("bl").adds(0, 2));
	adda(explabel(), new Coord(rx, prev.pos("ul").y), 1.0, 0.0);
	prev = add(new Label(LocalizationManager.tr("chr.sattr.learn_cost")), prev.pos("bl").adds(0, 2));
	adda(new RLabel<Integer>(() -> scost, Utils::thformat, n -> (n > chr.exp) ? debuff : Color.WHITE), new Coord(rx, prev.pos("ul").y), 1.0, 0.0);
	prev = adda(new Button(UI.scale(75), LocalizationManager.tr("chr.btn.buy")).action(this::buy), bframe.pos("ibr").subs(5, 5), 1.0, 1.0);
	adda(new Button(UI.scale(75), LocalizationManager.tr("chr.btn.reset")).action(this::reset), prev.pos("bl").subs(5, 0), 1.0, 1.0);
	pack();
    }
