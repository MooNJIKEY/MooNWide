...
public class GItem extends AWidget implements ItemInfo.SpriteOwner, GSprite.Owner, RandomSource {
    ...
    /** {@code null} until first {@link #info()} build; must not use {@link Collections#emptyList()} with {@code info == null} gate. */
	...
	if(this.info == null) {
	    ItemInfo.Raw raw = (this.rawinfo != null) ? this.rawinfo : ItemInfo.Raw.nil;
	...
	private void chstate(String nst) {
		...
		/* Match global Moon UI: never force vanilla wooden frames when minimal theme is on. */
		chdeco(Window.minimalTheme ? new Window.MinimalDeco() : new DefaultDeco(this.large));
		show();
		z(0);
		if(parent != null)
		    raise();
