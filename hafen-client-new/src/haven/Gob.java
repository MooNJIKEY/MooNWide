...
public class Gob implements RenderTree.Node, Sprite.Owner, Skeleton.ModOwner, EquipTarget, RandomSource {
    ...
    /** ConcurrentHashMap: hot {@link #getattr} path + safe iteration; still coordinate with {@link #setattr} via lock for slots/setupmods. */
    final Map<Class<? extends GAttrib>, GAttrib> attr = new ConcurrentHashMap<>();
    ...
    private static final float MOON_FLAT_CUPBOARD_ZSCALE = 0.58f;
	...
	public Coord3f getc(Coord2d rc, double ra) {
	    /* Always use surface Z (incl. flat-terrain mesh with relief). Forcing z=0 here hid
	     * placement previews and misaligned gobs vs rendered ground. */
	...
	private float getz(Coord2d rc, double ra) {
	    ...
	    if(Float.isNaN(ret))
		ret = (float)map.getz(surf, rc);
	    if(Float.isNaN(ret))
		ret = (float)map.getczraw(rc.x, rc.y);
	    return(ret);
	}
	...
	GAttrib ga = attr.get(attrclass(c));
	if(!c.isInstance(ga))
	    return(null);
	return(c.cast(ga));
    }
	...
	for(GAttrib ga : attr.values()) {
	    if(ga instanceof EquipTarget) {
		Supplier<? extends Pipe.Op> ret = ((EquipTarget)ga).eqpoint(nm, dat);
		if(ret != null)
		    return(ret);
	    }
	}
	...
	private GobState() {
	    ArrayList<Pipe.Op> ops = new ArrayList<>();
	    if(!setupmods.isEmpty()) {
		for(SetupMod mod : setupmods) {
		    Pipe.Op op = mod.gobstate();
		    if(op != null)
			ops.add(op);
		}
	    }
	    Pipe.Op sc = MoonGobScale.gobstate(Gob.this);
	    if(sc != null)
		ops.add(sc);
	    Pipe.Op status = MoonGobStatus.gobstate(Gob.this);
	    if(status != null)
		ops.add(status);
	    this.mods = ops.isEmpty() ? null : Pipe.Op.compose(ops.toArray(new Pipe.Op[0]));
	}
	...
	private class Placement implements Pipe.Op {
	    ...
	    final Coord3f oc, rc;
	    final Matrix4f rot;
	    final boolean flatwall;
	    final boolean flatcupboard;
		...
		try {
			...
			boolean flatwall = MoonConfig.flatWalls && MoonGobKind.isFlatWallTarget(Gob.this);
			boolean flatcupboard = MoonConfig.flatCupboards && MoonGobKind.isFlatCupboardTarget(Gob.this);
			if(flatwall)
			    rc.z -= 10.0f;
			else if(flatcupboard)
			    rc.z -= 9.0f;
			rc.y = -rc.y;
			this.flw = null;
			this.oc = oc;
			...
			this.flatwall = flatwall;
			this.flatcupboard = flatcupboard;
		    } else {
			this.flw = flwxf;
			this.oc = this.rc = null;
			this.rot = null;
			this.flatwall = false;
			this.flatcupboard = false;
		    }
		...
		if(this.flatwall != that.flatwall)
		    return(false);
		if(this.flatcupboard != that.flatcupboard)
		    return(false);
		...
		if(this.flw != null) {
		    ...
		    if(gndst == null) {
			if(flatcupboard)
			    gndst = Pipe.Op.compose(
				new Location(Transform.makexlate(new Matrix4f(), this.rc), "gobx"),
				new Location(rot, "gob"),
				Location.scale(1f, 1f, MOON_FLAT_CUPBOARD_ZSCALE));
			else
