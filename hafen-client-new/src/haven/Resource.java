...
public class Resource implements Serializable {
	...
	public InputStream get(String name) throws FileNotFoundException {
	    ...
	    if(name.equals("gfx/terobjs/cupboard") && !MoonConfig.flatCupboards)
		throw(new FileNotFoundException("flat cupboard override disabled: " + full));
	...
	/**
	 * Try this source before existing ones (e.g. localized {@code .res} trees under
	 * {@code ~/.haven/l10n/&lt;lang&gt;/}). Must be used before concurrent loads, or
	 * together with {@link #handle}'s snapshot (safe while loading).
	 */
	public void prepend(ResSource src) {
	    synchronized(sources) {
		((LinkedList<ResSource>)sources).addFirst(src);
	    }
	}
	...
	private void handle(Queued res) {
	    ResSource[] snap;
	    synchronized(sources) {
		snap = sources.toArray(new ResSource[sources.size()]);
	    }
	    for(ResSource src : snap) {
	...
	public Image(Message buf) {
	    ...
	    img = MoonResRuUiOverride.apply(Resource.this, id, img);
	    sz = Utils.imgsz(img);
	    so = UI.scale(o);
	    if(tsz == null)
		tsz = sz.add(o);
	...
	public AButton(Message buf) {
	    ...
	    name = buf.string();
	    req = buf.string(); /* Prerequisite skill */
	    hk = (char)buf.uint16();
	...
	sorted.sort(Comparator.comparing(res -> res.name));
