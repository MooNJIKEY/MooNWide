...
public class Window extends Widget {
    ...
    public static boolean minimalTheme = Utils.getprefb("moon-minimal-theme", true);
    public Deco deco;
    public String cap;
	...
	return(new MinimalDeco());
    }

    public static class MinimalDeco extends Deco {
	private static final int HDR = 22;
	private Text.Line captex;
	...
	protected int headerHeight() {
	    ...
	    return ((cap != null) && !cap.isEmpty()) ? UI.scale(HDR) : 0;
	}
	...
	protected void updateCap() {
	    ...
	    String c = wnd != null ? wnd.cap : null;
	    if(!java.util.Objects.equals(c, lastCap)) {
		lastCap = c;
		captex = (c != null) ? MoonUiTheme.title(c) : null;
	    }
	}

	protected String lockKey() {
	    Window wnd = wnd();
	    String cap = (wnd != null && wnd.cap != null && !wnd.cap.isEmpty()) ? wnd.cap : "window";
	    return "wlock-minimal-" + cap;
	}

	protected boolean locked() {
	    String key = lockKey();
	    return (key != null) && Utils.getprefb(key, false);
	}
	...
	public void iresize(Coord isz) {
	    resize(isz.add(0, headerHeight()));
	}

	public Area contarea() {
	    int hdr = headerHeight();
	    return Area.sized(Coord.of(0, hdr), sz.sub(0, hdr));
	}
	...
	protected boolean roundedChrome() {
	    Window wnd = wnd();
	    return (wnd instanceof CharWnd) || (wnd instanceof GameUI.Zergwnd);
	}
	...
	protected boolean hasHeaderMenu() {
	    return !buildHeaderMenuEntries().isEmpty();
	}

	protected Coord headerMenuAnchor() {
	    return MoonDropdownMenu.toRoot(this,
		MoonUiTheme.menuButtonCenter(sz, headerHeight(), hasLockButton(), hasCloseButton()));
	}
	...
	protected void resetWindowPosition() {
	    Window wnd = wnd();
	    if(wnd != null && wnd.parent != null) {
		wnd.c = wnd.parent.sz.sub(wnd.sz).div(2);
		wnd.savegeom();
	    }
	}

	protected boolean menuHit(Coord c) {
	    return hasHeaderMenu() && MoonUiTheme.menuButtonHit(c, sz, headerHeight(), hasLockButton(), hasCloseButton());
	}

	protected boolean lockHit(Coord c) {
	    return hasLockButton() && MoonUiTheme.lockButtonHit(c, sz, headerHeight(), hasCloseButton());
	}

	protected boolean closeHit(Coord c) {
	    return hasCloseButton() && MoonUiTheme.closeButtonHit(c, sz, headerHeight());
	}
	...
	public void draw(GOut g) {
	    ...
	    MoonUiTheme.drawPanelChrome(g, sz, hdr, (hdr > 0) ? captex : null, hasMenu, headerMenu != null,
		hasLock, locked(), hasClose, true, roundedChrome(),
		MoonUiTheme.BODY_TOP, MoonUiTheme.BODY_BOTTOM, MoonUiTheme.HEADER_TOP, MoonUiTheme.HEADER_BOTTOM);
	    super.draw(g);
	    if(hdr <= 0)
		MoonUiTheme.drawPanelActions(g, sz, hdr, hasMenu, headerMenu != null, hasLock, locked(), hasClose);
	}
	...
	public boolean mousedown(MouseDownEvent ev) {
	    ...
	    boolean hasClose = hasCloseButton();
	    int hdr = headerHeight();
	    if(ev.b == 1 && hasClose && MoonUiTheme.closeButtonHit(ev.c, sz, hdr)) {
		onCloseClicked();
		return true;
	    }
	    if(ev.b == 1 && hasLock && MoonUiTheme.lockButtonHit(ev.c, sz, hdr, hasClose)) {
		toggleLock();
		return true;
	    }
	    if(ev.b == 1 && hasMenu && MoonUiTheme.menuButtonHit(ev.c, sz, hdr, hasLock, hasClose)) {
		toggleHeaderMenu();
		return true;
	    }
		...
		return true;
	    if(ev.b == 1 && ev.c.y < ((hdr > 0) ? hdr : UI.scale(8))) {
		Window wnd = wnd();
		if(wnd != null) {
		    if(wnd.parent != null)
	...
	super.cdestroy(w);
	MoonStorage.onWindowChildRemoved(this, w);
    }
	...
	if(msg == "cap") {
	    ...
	    MoonMisc.onWindowCapChanged(this);
	    MoonStorage.onWindowCapChanged(this);
	} else if(msg == "dhide") {
