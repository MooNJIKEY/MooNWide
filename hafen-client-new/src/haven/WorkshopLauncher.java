...
public class WorkshopLauncher {
    ...
    private static Path safeResolveWithin(Path root, String rel) throws IOException {
	if(rel == null || rel.isEmpty())
	    throw(new MessageException("empty relative path"));
	if(rel.indexOf('\0') >= 0)
	    throw(new MessageException("invalid path"));
	Path base = root.toAbsolutePath().normalize();
	Path resolved = base.resolve(rel).normalize();
	if(!resolved.startsWith(base))
	    throw(new MessageException("path escapes client directory"));
	return(resolved);
    }
	...
	if((desc = cl.props.getProperty("launcher")) != null) {
	    Path lf = safeResolveWithin(cl.path, desc);
		...
		if(js.isEmpty())
		    continue;
		Path cp = safeResolveWithin(cl.path, js);
