...
public class CharWnd extends Window {
    private static final Coord TAB_AREA = UI.scale(new Coord(506, 385));
    private static final Coord CHR_OUTER_PAD = UI.scale(new Coord(15, 16));
    public static final RichText.Foundry ifnd = new RichText.Foundry(RichText.ImageSource.res(Resource.remote()), java.awt.font.TextAttribute.FAMILY, "SansSerif", java.awt.font.TextAttribute.SIZE, UI.scale(11)).aa(true);
    private static final java.awt.Font uihead = Text.serif.deriveFont(java.awt.Font.BOLD, (float)Math.floor(UI.scale(24.0)));
    private static final java.awt.Font uibody = Text.serif.deriveFont(java.awt.Font.BOLD, (float)Math.floor(UI.scale(19.0)));
    public static final Text.Furnace catf = new BlurFurn(new TexFurn(new Text.Foundry(uihead).aa(true), Window.ctex), UI.scale(3), UI.scale(2), new Color(54, 34, 22));
    public static final Text.Furnace failf = new BlurFurn(new TexFurn(new Text.Foundry(uihead).aa(true), Resource.loadimg("gfx/hud/fontred")), UI.scale(3), UI.scale(2), new Color(54, 34, 22));
    public static final Text.Foundry attrf = new Text.Foundry(uibody).aa(true);
	...
	public List<ItemInfo> info() {
		...
		if(!binfo.isEmpty())
		    binfo.add(new ItemInfo.Name(this, LocalizationManager.autoTranslateProcessed(attr.res().get().flayer(Resource.tooltip).t)));
		this.binfo = binfo;
	    }
	    return(this.binfo);
	}
	...
	super(UI.scale(new Coord(300, 290)), LocalizationManager.tr("chr.title"));

	/* Always Moon chrome — do not follow global vanilla {@link Window#minimalTheme} for this sheet. */
	chdeco(new Window.MinimalDeco());

	Tabs tabs = new Tabs(new Coord(15, 10), TAB_AREA, this);
        battrtab = tabs.add();
        sattrtab = tabs.add();
	skilltab = tabs.add();
	fighttab = tabs.add();
...

	{
		...
		new TB("battr", battrtab, LocalizationManager.tr("chr.tab.battr")),
		new TB("sattr", sattrtab, LocalizationManager.tr("chr.tab.sattr")),
		new TB("skill", skilltab, LocalizationManager.tr("chr.tab.skill")),
		new TB("fgt",   fighttab, LocalizationManager.tr("chr.tab.fight")),
		new TB("wound", woundtab, LocalizationManager.tr("chr.tab.wound")),
		new TB("quest", questtab, LocalizationManager.tr("chr.tab.quest"))
	    );
	}

	resize(contentsz().add(CHR_OUTER_PAD));
    }
	...
	if((place == "tab") || /* XXX: Remove me! */ Utils.eq(args[0], Coord.of(47, 47))) {
		...
		if(gui != null && gui.fepmeter == null) {
		    gui.addClientMeter(gui.fepmeter = new FepMeter(battr.feps));
		    gui.fepmeter.show(Utils.getprefb("fepmeter", true));
		}
