...
public class RemoteUI implements UI.Receiver, UI.Runner {
	...
	/* Outgoing mutation happens in {@link UI#wdgmsg} via {@link MoonPacketHook#fireOutgoing}. */
	...
	try {
		...
		if(msg == null) {
		    ...
		    Session next = ((Return)msg).ret;
		    MoonConnectLog.log("world: RemoteUI handoff to new game session user=" + next.user.name);
		    sess.close();
		    return(new RemoteUI(next));
		} else if(msg.type == RMessage.RMSG_NEWWDG) {
		    int id = msg.int32();
		    String type = msg.string();
		    int parent = msg.int32();
		    if("sess".equals(type))
			MoonConnectLog.log("remote: server NEWWDG type=sess id=" + id + " parent=" + parent);
