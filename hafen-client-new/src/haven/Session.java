...
public class Session implements Resource.Resolver {
    ...
    public volatile SignKey sesskey;
	...
	try {
	    MoonPacketHook.fireIncoming(msg.type, msg.clone());
	} catch(Throwable t) {
	    new Warning(t, "MoonPacketHook.fireIncoming").issue();
	}
	try {
	    msg = MoonPacketHook.applyIncomingTransformers(this, msg);
	    if(msg == null)
		return;
	} catch(Throwable t) {
	    new Warning(t, "MoonPacketHook.applyIncomingTransformers").issue();
	}
	MoonConnectLog.logInboundRelUi(msg);
	...
	MoonConnectLog.log("session: open channel to " + server + " user=" + user.name);
	this.conn = Connection.openWithFailover(server);
	this.user = user;
	this.glob = new Glob(this);
	conn.add(conncb);
	MoonConnectLog.log("session: UDP handshake (MSG_SESS) to " + server);
	conn.connect((user.alias != null) ? user.alias : user.name, encrypt, cookie, args);
	MoonConnectLog.log("session: handshake OK " + server);
	MoonConnectLog.hint(
	    "This is the first game UDP session (login / menu). After Play you should see net: inbound NEWWDG type=sess then play: lines. If you only see client: queued wdgmsg play with no sess, the game server did not open a world session (character/account/subscription, or try the official client). Also watch net: inbound WDGMSG name=err.",
	    "Pervyj UDP-seans (menju). Posle Play dolzhny pojavitsja net: inbound NEWWDG type=sess i play:. Esli tolko client: play bez sess - server ne otkryl mir (personazh/akkaunt/poddiska). Sm. net: inbound WDGMSG name=err.");
    ...
    /**
     * Dev: ставит в очередь исходящий REL {@code WDGMSG} как при Shift+клике по {@link ISBox} —
     * {@code xfer} на указанный виджет, ждёт фактической отправки REL по UDP, затем рвёт канал.
     * Игровой хоткей: {@code J} в {@link GameUI#globtype}.
     *
     * @param xferWidgetId привязанный id виджета (например {@link GameUI#maininv} через {@link UI#widgetid}).
     */
    public void crashTest(int xferWidgetId) {
	PMessage msg = new PMessage(RMessage.RMSG_WDGMSG);
	msg.addint32(xferWidgetId);
	msg.addstring("xfer");
	msg.addlist();
	queuemsg(msg);
	conn.waitLastRelSendAttempt(0.4);
	conn.crashCloseSocketNow();
    }
