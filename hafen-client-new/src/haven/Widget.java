...
public class Widget {
	...
	if(focusctl) {
		...
		if(hasfocus) {
		    ...
		    w.gotfocus();
		    if(last != null)
			new FocusChangeEvent(last, false).dispatch(last);
		    if(last != null)
			new LostFocusEvent(last).dispatch(last);
		    if(w != null)
			new FocusChangeEvent(w, true).dispatch(w);
		    if(w != null)
			new GotFocusEvent(w).dispatch(w);
    ...
    public static class FocusChangeEvent extends Event {
	public final Widget widget;
	public final boolean gained;

	public FocusChangeEvent(Widget widget, boolean gained) {
	    this.widget = widget;
	    this.gained = gained;
	}
    ...
    public static class GotFocusEvent extends Event {
	public final Widget widget;

	public GotFocusEvent(Widget widget) {
	    this.widget = widget;
	}
    ...
    public static class LostFocusEvent extends Event {
	public final Widget widget;

	public LostFocusEvent(Widget widget) {
	    this.widget = widget;
	}
	...
	public Tex get() {
	    long g = LocalizationManager.autoTranslateUiGeneration();
	    if(!hasrend || rendGen != g) {
		render: {
		    try {
			...
			if(text != null)
			    text = LocalizationManager.autoTranslateProcessed(text);
	...
	public Tex get() {
	    ...
	    long g = LocalizationManager.autoTranslateUiGeneration();
	    if(!hrend || (rkey != key) || rendGen != g) {
		String tip;
		int w = 0;
		if(base != null) {
		    String b = base;
		    b = LocalizationManager.autoTranslateProcessed(b);
		    if(rich) {
			tip = b;
