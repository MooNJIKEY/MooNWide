...
public class MCache implements MapSource {
	...
	public double getfzraw(Coord c) {return(getz(c));}
    ...
    public double getfzraw(Coord tc) {
	Grid g = getgridt(tc);
	return(g.getz(tc.sub(g.ul)));
    }

    private double getfzReal(Coord tc) {
	return(getfzraw(tc));
    }

    /**
     * Bilinear height from raw grid corners (no MooN flat-terrain vertex blend).
     * Fractions use {@link Utils#floormod(double, double)} for stable behaviour at negative coords.
     */
    public double getczraw(double px, double py) {
	double tw = tilesz.x, th = tilesz.y;
	...
	return(((1.0 - sy) * (((1.0 - sx) * getfzraw(ul)) + (sx * getfzraw(ul.add(1, 0))))) +
	       (sy * (((1.0 - sx) * getfzraw(ul.add(0, 1))) + (sx * getfzraw(ul.add(1, 1))))));
    }

    /** One flattened vertex (same formula as {@link #getfz} when flat terrain is on). */
    private static double flatVertexZ(double relief, double zAt, double r00, double r10, double r01, double r11) {
	double ref = (r00 + r10 + r01 + r11) * 0.25;
	return(ref + (zAt - ref) * relief);
    }

    /**
     * Continuous surface height at (px,py): bilinear interpolation of {@link #getfz} at tile corners.
     * When {@link MoonConfig#flatTerrain} is on, this matches the same vertex field as {@link #zsurf} /
     * map mesh. Uses a 3×3 raw-height stencil (9 {@link #getfzraw} calls) instead of nesting four
     * {@link #getfz} calls (~20 raw samples), which was heavy and could destabilize the client.
     */
    public double getcz(double px, double py) {
	if(MoonConfig.flatTerrain)
	    return(0);
	return(getczraw(px, py));
    }
	...
	for(Map.Entry<Coord, Grid> e : grids.entrySet()) {
	    try { e.getValue().invalidate(); } catch(Exception ignored) {}
	}
    }
	...
	synchronized(req) {
		...
		if(now - r.lastreq > 1000) {
		    r.lastreq = now;
		    /* Vanilla stopped after ~4 sends per grid — too short for slow spawns (new char / wisp).
		     * Keep retrying much longer; drop only to avoid unbounded stale keys. */
		    if(++r.reqs > 600) {
			i.remove();
			updated = true;
		    } else {
