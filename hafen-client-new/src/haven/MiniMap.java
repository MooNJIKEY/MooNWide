...
public class MiniMap extends Widget {
    ...
    private static final double ICON_SCAN_VISIBLE_INTERVAL = 0.12;
    private static final double ICON_SCAN_HIDDEN_INTERVAL = 0.45;
    private static final int TP_MARK_R = UI.scale(5);
    private static final int DEFAULT_MIN_ZOOM_LEVEL = -4;
    private static final int DEFAULT_MAX_ZOOM_LEVEL = 10;
    ...
    private double lastIconScan = Double.NEGATIVE_INFINITY;
	...
	double scanInterval = visible() ? ICON_SCAN_VISIBLE_INTERVAL : ICON_SCAN_HIDDEN_INTERVAL;
	if((now - lastIconScan) >= scanInterval) {
	...
	return(UI.unscale(levelScale(dlvl)));
    }
    ...
    private static int gridLevel(int level) {
	return(Math.max(level, 0));
    }

    private static Coord scaledWorldSpan(Coord base, float scale) {
	return(Coord.of(Math.max(1, (int)Math.ceil(base.x * scale)),
			Math.max(1, (int)Math.ceil(base.y * scale))));
    }

    private Coord drawGridSize(Coord imgsz) {
	if(dlvl >= 0)
	    return(UI.scale(imgsz));
	int mul = 1 << -dlvl;
	return(UI.scale(Coord.of(imgsz.x * mul, imgsz.y * mul)));
    }

    public Coord st2c(Coord tc) {
	return(UI.scale(tc.add(sessloc.tc).sub(dloc.tc).div(scalef())).add(sz.div(2)));
    }
	...
	Coord hsz = sz.div(2);
	int glvl = gridLevel(zoomlevel);
	Coord zmaps = cmaps.mul(1 << glvl);
	Coord worldSpan = scaledWorldSpan(UI.unscale(sz), UI.unscale(levelScale(zoomlevel)));
	Area next = Area.sized(loc.tc.sub(worldSpan.div(2)).div(zmaps),
	    worldSpan.div(zmaps).add(2, 2));
	...
	if(file.lock.readLock().tryLock()) {
		...
		for(Coord c : dgext) {
			...
			display[dgext.ri(c)] = new DisplayGrid(dloc.seg, c, glvl, dloc.seg.grid(glvl, c.mul(1 << glvl)));
		}
	    } finally {
		file.lock.readLock().unlock();
	    }
	...
	try {
	    Tex img = disp.img();
	    if(img != null)
		g.image(img, ul, drawGridSize(img.sz()));
	} catch(Loading l) {
	}
    }
	...
	for(Coord c : dgext) {
	    ...
	    if(disp == null)
		continue;
	    /* Negative zoom levels enlarge a base grid image; use the grid's actual map extent so
	     * the origin stays stable while zooming in instead of sliding off-screen. */
	    Coord ul = UI.scale(disp.mapext.ul.sub(dloc.tc).div(scalef())).add(hsz);
	    drawgrid(g, ul, disp);
	}
    }
    ...
    protected Coord teleportPointScreen(TeleportPoint point) {
	if(point == null)
	    return(null);
	Location dloc = this.dloc;
	if(dloc == null)
	    return(null);
	Coord mapTc;
	if(point.hasMapLocation()) {
	    if(point.mapSeg != dloc.seg.id)
		return(null);
	    mapTc = point.mapTc();
	} else {
	    GameUI gui = getparent(GameUI.class);
	    mapTc = TeleportManager.inferPointMapTc(gui, point, dloc.seg.id);
	    if(mapTc == null)
		return(null);
	}
	return(mapTc.sub(dloc.tc).div(scalef()).add(sz.div(2)));
    }

    protected TeleportPoint teleportPointAt(Coord sc) {
	List<TeleportPoint> pts = TeleportManager.getPointsSnapshot();
	for(int i = pts.size() - 1; i >= 0; i--) {
	    TeleportPoint point = pts.get(i);
	    Coord pc = teleportPointScreen(point);
	    if(pc == null)
		continue;
	    if(Math.abs(sc.x - pc.x) <= TP_MARK_R + 2 && Math.abs(sc.y - pc.y) <= TP_MARK_R + 2)
		return(point);
	}
	return(null);
    }

    protected void drawTeleportPoints(GOut g) {
	for(TeleportPoint point : TeleportManager.getPointsSnapshot()) {
	    Coord pc = teleportPointScreen(point);
	    if(pc == null)
		continue;
	    java.awt.Color col = TeleportManager.pointColor(point);
	    Coord ul = pc.sub(TP_MARK_R, TP_MARK_R);
	    g.chcolor(18, 12, 30, 220);
	    g.frect(ul.sub(1, 1), Coord.of((TP_MARK_R * 2) + 2, (TP_MARK_R * 2) + 2));
	    g.chcolor(col);
	    g.frect(ul, Coord.of(TP_MARK_R * 2, TP_MARK_R * 2));
	    g.chcolor(255, 255, 255, 220);
	    g.rect(ul, Coord.of(TP_MARK_R * 2, TP_MARK_R * 2));
	    g.chcolor();
	    if(point.name != null && !point.name.isEmpty())
		g.aimage(point.labelTex(), pc.add(0, -UI.scale(8)), 0.5, 1.0);
	}
    }
    ...
    /** Max zoom-out level (higher = smaller map scale). */
    protected int maxZoomLevel() {
	return(DEFAULT_MAX_ZOOM_LEVEL);
    }

    /** Min zoom-in level (lower = closer map scale). */
	...
	/* First paint / sparse map: vanilla returned false here so wheel never zoomed out. */
	if(dext == null)
	    return(true);
	try {
	...
	/* Edge tiles still loading: still allow zoom out (edges may show loading briefly). */
	return(true);
    }

    protected boolean allowzoomin() {
	...
	for(DisplayIcon icon : icons) {
		...
		try {
			...
			if(icon.conf.getmarkp()) {
			    ...
			    String tipt = (tt != null) ? tt.t : "";
			    mid = new SMarker(info.seg, sc, tipt, 0, new Resource.Saved(Resource.remote(), micon.res.name, micon.res.ver));
			    file.add(mid);
			} else {
			    mid = null;
			}
    ...
    private TeleportPoint dstp;
    public boolean mousedown(MouseDownEvent ev) {
	dsloc = xlate(ev.c);
	if(dsloc != null) {
	    dstp = teleportPointAt(ev.c);
	    if((dstp != null) && clicksavedpoint(dstp, dsloc, ev.b, true))
		return(true);
	    dsicon = iconat(ev.c);
	...
	release: if(!dragging && (dsloc != null)) {
	    if((dstp != null) && clicksavedpoint(dstp, dsloc, ev.b, false))
		break release;
	...
	if(ev.a > 0) {
	    if(allowzoomout())
		zoomlevel = Math.min(zoomlevel + 1, Math.min(maxZoomLevel(), dlvl + 1));
	} else {
	    if(allowzoomin())
	...
	TeleportPoint point = teleportPointAt(c);
	if(point != null)
	    return(Text.render(point.name + " [" + LocalizationManager.tr("tp.cat." + point.category) + "]"));
	if(dloc != null) {
    ...
    public boolean clicksavedpoint(TeleportPoint point, Location loc, int button, boolean press) {
	if(button != 1)
	    return(false);
	if(press)
	    return(true);
	GameUI gui = getparent(GameUI.class);
	return TeleportManager.tryNavigateTo(gui, point, "map-saved:" + point.name);
    }
	...
	if((sessloc != null) && (sessloc.seg == loc.seg)) {
	    ...
	    int mf = ui.modflags();
	    int sendMf = mf & ~UI.MOD_META;
	    Coord2d clickRc = loc.tc.sub(sessloc.tc).mul(tilesz);
	    if(gob == null)
		mv.wdgmsg("click", mc,
			  clickRc.floor(posres),
			  button, sendMf);
	    else
		mv.wdgmsg("click", mc,
			  clickRc.floor(posres),
