...
public class BAttrWnd extends Widget {
    ...
    private static Tex headerTex(String assetKey, String textKey, int maxW, int maxH) {
	return(MoonUiTheme.labelTex(assetKey, LocalizationManager.tr(textKey), maxW, maxH));
    }

    /** Swaps bilingual fantasy banner textures when the active MooNWide language changes. */
    private static final class HeaderImg extends Img {
	private final String assetKey;
	private final String textKey;
	private final int maxW;
	private final int maxH;
	private String langTracked = MoonL10n.lang();

	HeaderImg(String assetKey, String textKey, int maxW, int maxH) {
	    super(headerTex(assetKey, textKey, maxW, maxH));
	...
	public void tick(double dt) {
	    super.tick(dt);
	    String l = MoonL10n.lang();
	    if(!l.equals(langTracked)) {
		langTracked = l;
		setimg(headerTex(assetKey, textKey, maxW, maxH));
	    }
	}
    }
	...
	private Attr(Glob glob, String attr, Color bg) {
	    ...
	    this.rnm = attrf.render(LocalizationManager.autoTranslateProcessed(res.flayer(Resource.tooltip).t));
	...
	public void tick(double dt) {
		...
		try {
		    ...
		    ui.loader.defer(() -> ui.msg(String.format(LocalizationManager.tr("chr.foodmeter.gained"), trev.get().flayer(Event.class).nm), Color.WHITE, null), null);
	...
	public Object tooltip(Coord c, Widget prev) {
		...
		cur = ItemInfo.catimgs(0, cur, Text.render(String.format(LocalizationManager.tr("chr.foodmeter.total"), Utils.odformat2(sum, 2), Utils.odformat(cap, 2))).img);
		rtip = new TexI(cur);
	    }
	    return(rtip);
	}
	...
	public Object tooltip(Coord c, Widget prev) {
	    if(rtip == null) {
	    rtip = RichText.render(String.format(LocalizationManager.tr("chr.foodmeter.tip"), lbl, glut * 1000, Math.round(gmod * 100)), -1).tex();
	    }
	    return(rtip);
	}
    }
	...
	Widget prev;
	prev = add(CharWnd.settip(new HeaderImg("chr-hdr-base", "chr.hdr.base", CharWnd.width, UI.scale(58)), "gfx/hud/chr/tips/base"), Coord.z);
	attrs = new ArrayList<>();
	Attr aw;
	attrs.add(aw = add(new Attr(glob, "str", every), prev.pos("bl").adds(8, 2).add(wbox.btloff())));
	...
	cons = add(new Constipations(Coord.of(attrw, ah)), prev.pos("bl").adds(8, 2).add(wbox.btloff()));
	prev = Frame.around(this, Collections.singletonList(cons));
	prev = add(CharWnd.settip(new HeaderImg("chr-hdr-hunger", "chr.hdr.hunger", width, UI.scale(58)), "gfx/hud/chr/tips/hunger"), prev.pos("bl").x(width).adds(0, 12));
	glut = add(new GlutMeter(), prev.pos("bl").adds(8, 2));
	pack();
    }
