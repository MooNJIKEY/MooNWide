...
public class JOGLPanel extends GLCanvas implements GLPanel, Console.Directory {
    /** Full BGL command dumps on GL errors are huge; only when {@link GLPanel.Loop#gldebug}. */
	...
	addGLEventListener(new GLEventListener() {
		...
		public void reshape(GLAutoDrawable wdg, int x, int y, int w, int h) {
		    JOGLPanel.this.applyDrawableShape(wdg, w, h);
		}
	...
	GLPanelHolder.instance = this;
	newui(null);
    }

    /**
     * JOGL's reshape callback can report a letterboxed sub-rect; AWT component size is the
     * authoritative drawable size when the window is maximized. Merge all sources.
     */
    private void applyDrawableShape(GLAutoDrawable wdg, int rw, int rh) {
	synchronized(this) {
	    int sw = Math.max(1, wdg.getSurfaceWidth());
	    int sh = Math.max(1, wdg.getSurfaceHeight());
	    int aw = Math.max(1, getWidth());
	    int ah = Math.max(1, getHeight());
	    int cw = sw;
	    int ch = sh;
	    cw = Math.max(cw, aw);
	    ch = Math.max(ch, ah);
	    cw = Math.max(cw, Math.max(1, rw));
	    ch = Math.max(ch, Math.max(1, rh));
	    commitViewportShape(Area.sized(Coord.of(cw, ch)));
	}
    }

    private void commitViewportShape(Area area) {
	if(area == null || !area.positive())
	    return;
	if(shape != null && shape.equals(area))
	    return;
	shape = area;
	wnd = base.copy();
	wnd.prep(new States.Viewport(shape)).prep(new Ortho2D(shape));
    }
	...
	synchronized(this) {
	    ...
	    commitViewportShape(Area.sized(Coord.of(w, h)));
	}
    }
	...
	GLEnvironment env;
	synchronized(this) {
	    /* Keep viewport in sync with peer size every frame (reshape vs setBounds ordering on Windows). */
	    {
