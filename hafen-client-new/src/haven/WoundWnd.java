...
public class WoundWnd extends Widget {
	...
	public void layout(Layout l) {
	    BufferedImage t = ifnd.render(LocalizationManager.autoTranslateProcessed(str), l.width).img;
	    if(t != null) {
	...
	public String name() {
	    ...
	    return((nm == null) ? null : nm.nameText());
	}
	...
	public void tick(double dt) {
	    ...
	    catch(Resource.LoadFailedException | Resource.LoadException e) {}
	}
	...
	public BufferedImage renderinfo(int width) {
	    ...
	    ItemInfo.Name nm = ItemInfo.find(ItemInfo.Name.class, info);
	    if(nm != null)
		l.cmp.add(namef.render(nm.nameText()).img, Coord.of(0, l.cmp.sz.y + UI.scale(10)));
	    l.cmp.sz = l.cmp.sz.add(0, UI.scale(10));
	...
	public void tick(double dt) {
		...
		for(Wound w : wounds) {
		    try {
			w.sortkey = LocalizationManager.autoTranslateProcessed(w.res.get().flayer(Resource.tooltip).t);
		    } catch(Loading l) {
			w.sortkey = "\uffff";
			loading = true;
		    } catch(Resource.LoadFailedException | Resource.LoadException e) {
	...
	prev = add(CharWnd.settip(new MoonLocalizedLabel("chr-hdr-wounds", "wound.hdr", CharWnd.width, UI.scale(58)), "gfx/hud/chr/tips/wounds"), 0, 0);
