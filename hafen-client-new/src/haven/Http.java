...
public class Http {
    ...
    /** When true, HTTPS checks certificate hostname (default false matches legacy ignoreName). */
    public static final Config.Variable<Boolean> sslVerifyHostname = Config.Variable.propb("haven.ssl.verify-hostname", false);
	...
	URL target = url;
	String host = Utils.connecthost(url.getHost());
	if(host != null)
	    target = new URL(url.getProtocol(), host, url.getPort(), url.getFile());
	URLConnection conn;
	if(target.getProtocol().equals("https"))
	    conn = ssl.connect(target);
	else
