...
public class AuthClient implements Closeable {
	...
	boolean fin = false;
	sk = HavenNetProxy.connectAuthTcp(srv);
	try {
	    ByteChannel bottom = sk;
	    if(HavenDpiBypass.authExtraSplitLayer()) {
		int bud = Utils.getprefi("haven-auth-dpi-split-budget", 6144);
		if(bud > 0)
		    bottom = new DpiSplitWriteChannel(sk, bud);
	    }
	    ssk = new SslChannel(obf ? new Obfuscation(bottom) : bottom, ssl.engine(srv.host, srv.port));
	    ssk.handshake();
	    if(strictcert.get())
	...
	boolean obfFirst = HavenDpiBypass.authObfFirst();
	if(obfFirst) {
	    try {
		connect(srv, true);
	    } catch(IOException e) {
	...
	try {
	    SocketAddress sa = HavenDpiBypass.remoteAddressOrNull(sk);
	    if(sa != null)
		return sa;
	} catch(IOException e) {
	    throw(new RuntimeException(e));
	}
	throw(new RuntimeException("no remote address for auth connection"));
    }
	...
	private byte[] hashpw(AuthClient cl) throws IOException {
		...
		Object[] spec = ((pwdata.length == 2) && (pwdata[1] instanceof Object[])) ? (Object[])pwdata[1] : Utils.splice(pwdata, 1);
		return(prehash(pw, spec));
