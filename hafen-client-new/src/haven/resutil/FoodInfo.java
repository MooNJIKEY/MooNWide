...
public class FoodInfo extends ItemInfo.Tip {
	...
	String head = String.format(LocalizationManager.tr("foodinfo.stat_line"), Utils.odformat2(end * 100, 2), Utils.odformat2(glut * 1000, 2));
	if(cons != 0)
	    head += String.format(LocalizationManager.tr("foodinfo.satiation_suffix"), Utils.odformat2(cons * 100, 2));
	l.cmp.add(RichText.render(head, 0).img, Coord.of(0, l.cmp.sz.y));
	for(int i = 0; i < evs.length; i++) {
	    Color col = Utils.blendcol(evs[i].ev.col, Color.WHITE, 0.5);
	    l.cmp.add(catimgsh(5, evs[i].img, RichText.render(String.format("%s: %s{%s}", MoonL10n.game(evs[i].ev.nm), RichText.Parser.col2a(col), Utils.odformat2(evs[i].a, 2)), 0).img),
		      Coord.of(UI.scale(5), l.cmp.sz.y));
	}
	if(sev > 0)
	    l.cmp.add(RichText.render(String.format(LocalizationManager.tr("foodinfo.total_line"), Utils.odformat2(sev, 2), Utils.odformat2(sev / (1000 * glut), 2)), 0).img,
		      Coord.of(UI.scale(5), l.cmp.sz.y));
	for(int i = 0; i < efs.length; i++) {
	    BufferedImage efi = ItemInfo.longtip(efs[i].info);
	    if(efs[i].p != 1)
		efi = catimgsh(5, efi, RichText.render(String.format(LocalizationManager.tr("foodinfo.effect_chance"), (int)Math.round(efs[i].p * 100)), 0).img);
