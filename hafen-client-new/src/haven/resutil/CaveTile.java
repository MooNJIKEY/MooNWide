...
public class CaveTile extends Tiler {
	...
	public Vertex[] fortile(Coord tc) {
	    if(wv[cs.o(tc)] == null) {
		if(MoonConfig.flatCaves) {
		    Vertex[] buf = wv[cs.o(tc)] = new Vertex[2];
		    buf[0] = ms.new Vertex(ms.fortile(tc));
		    buf[1] = ms.new Vertex(buf[0].x, buf[0].y, buf[0].z + 7.0f);
		} else {
			...
			if(!MoonConfig.straightCaveWalls) {
    ...
    /** Bottom segment only (same geometry as {@code i == 0} in {@link #modelwall}) — fills edge when full walls are hidden. */
    ...
    /** First vertical band of {@link #wtex} only — avoids black void at cave edges when {@link MoonConfig#hideCaveWalls} is on. */
