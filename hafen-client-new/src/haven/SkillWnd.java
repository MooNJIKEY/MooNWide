...
public class SkillWnd extends Widget {
	...
	public String rendertext() {
	    ...
	    buf.append("$b{$font[serif,16]{" + LocalizationManager.autoTranslateProcessed(res.flayer(Resource.tooltip).t) + "}}\n\n\n");
	    if(cost > 0)
		buf.append(String.format(LocalizationManager.tr("skill.detail.cost"), cost));
	    buf.append(LocalizationManager.autoTranslateProcessed(res.flayer(Resource.pagina).text));
	    return(buf.toString());
	}
	...
	public Text tooltip() {
	    if(tooltip == null)
		tooltip = Text.render(LocalizationManager.autoTranslateProcessed(res.get().flayer(Resource.tooltip).t));
	    return(tooltip);
	}
    }
	...
	public String rendertext() {
	    ...
	    buf.append("$b{$font[serif,16]{" + LocalizationManager.autoTranslateProcessed(res.flayer(Resource.tooltip).t) + "}}\n\n\n");
	    if(score > 0)
		buf.append(LocalizationManager.tr("skill.exp.line") + Utils.thformat(score) + "\n\n");
	    buf.append(LocalizationManager.autoTranslateProcessed(res.flayer(Resource.pagina).text));
	    return(buf.toString());
	}
	...
	public SkillGrid(Coord sz) {
	    super(sz);
	    nsk = new Group(UI.scale(40, 40), new Coord(-1, 5), LocalizationManager.tr("skill.grp.avail"), Collections.emptyList());
	    csk = new Group(UI.scale(40, 40), new Coord(-1, 5), LocalizationManager.tr("skill.grp.known"), Collections.emptyList());
	...
	private void sksort(List<Skill> skills) {
		for(Skill sk : skills) {
		    try {
			sk.sortkey = LocalizationManager.autoTranslateProcessed(sk.res.get().flayer(Resource.tooltip).t);
		    } catch(Loading l) {
		    sk.sortkey = sk.nm;
		    loading = true;
		}
	...
	public final Text.Foundry prsf = new Text.Foundry(Text.serif.deriveFont(java.awt.Font.BOLD, (float)UI.scale(15.0))).aa(true);
	...
	public CredoGrid(Coord sz) {
	    super(sz);
	    pcrc = new Img(GridList.dcatf.render(LocalizationManager.tr("credo.pursuing")).tex());
	    ncrc = new Img(GridList.dcatf.render(LocalizationManager.tr("credo.avail")).tex());
	    ccrc = new Img(GridList.dcatf.render(LocalizationManager.tr("credo.acquired")).tex());
	    pbtn = new Button(btnw, LocalizationManager.tr("credo.pursue"), false) {
		    public void click() {
			if(sel != null)
			    credo.wdgmsg("crpursue", sel.nm);
		    }
		};
	    qbtn = new Button(btnw, LocalizationManager.tr("credo.showq"), false) {
		    public void click() {
	...
	private class CredoImg extends Img {
		...
		super(crtex(cr));
		this.cr = cr;
		this.tooltip = Text.render(LocalizationManager.autoTranslateProcessed(cr.res.get().flayer(Resource.tooltip).t));
	    }
	...
	private void sort(List<Credo> buf) {
	    Collections.sort(buf, Comparator.comparing(cr -> LocalizationManager.autoTranslateProcessed(cr.res.get().flayer(Resource.tooltip).t)));
	}
	...
	private void update() {
		...
		cont.add(new Label(String.format(LocalizationManager.tr("credo.level_fmt"), pcl, pclt), prsf), pcrim.c.x + pcrim.sz.x + m, y);
		cont.add(new Label(String.format(LocalizationManager.tr("credo.quest_fmt"), pcql, pcqlt), prsf), pcrim.c.x + pcrim.sz.x + m, y + UI.scale(20));
		...
		if(pcr == null) {
		    cont.add(pbtn, m, y);
		    if(cost > 0)
			cont.adda(new Label(String.format(LocalizationManager.tr("credo.cost_lp_fmt"), cost)), pbtn.c.x + pbtn.sz.x + UI.scale(10), pbtn.c.y + (pbtn.sz.y / 2), 0, 0.5);
		    y += pbtn.sz.y;
		}
		y += UI.scale(10);
	    }
	...
	public void tick(double dt) {
		...
		for(Experience exp : seen.items) {
		    try {
			exp.sortkey = LocalizationManager.autoTranslateProcessed(exp.res.get().flayer(Resource.tooltip).t);
		    } catch(Loading l) {
			exp.sortkey = "\uffff";
			loading = true;
		    }
	...
	prev = add(CharWnd.settip(new MoonLocalizedLabel("chr-hdr-lore", "skill.hdr.lore", CharWnd.width, UI.scale(58)), "gfx/hud/chr/tips/skills"), Coord.z);
	LoadingTextBox info = add(new LoadingTextBox(new Coord(attrw, height), "", ifnd), prev.pos("bl").adds(8, 2).add(wbox.btloff()));
	info.bg = new Color(0, 0, 0, 128);
	Frame.around(this, Collections.singletonList(info));

	prev = add(new MoonLocalizedLabel("chr-hdr-entries", "skill.hdr.entries", CharWnd.width, UI.scale(58)), width, 0);
	Tabs lists = new Tabs(prev.pos("bl").adds(8, 2), new Coord(attrw + wbox.bisz().x, 0), this);
	int gh = UI.scale(241);
	Tabs.Tab sktab = lists.add();
	{
	    ...
	    Button bbtn = sktab.adda(new Button(UI.scale(50), LocalizationManager.tr("skill.buy")).action(() -> {
			if (skg.sel != null)
			    skill.wdgmsg("buy", skg.sel.nm);
	    }), bf.pos("ibr").subs(10, 0).y(bf.pos("mid").y), 1.0, 0.5);
	    Label clbl = sktab.adda(new Label(LocalizationManager.tr("skill.cost_lbl")), bf.pos("iul").adds(10, 0).y(bf.pos("mid").y), 0, 0.5);
	    sktab.adda(new RLabel<Pair<Integer, Integer>>(() -> new Pair<>(((skg.sel == null) || skg.sel.has) ? null : skg.sel.cost, this.chr.exp),
							  n -> (n.a == null) ? LocalizationManager.tr("skill.cost_na") : String.format(LocalizationManager.tr("skill.cost_lp_fmt"), n.a, n.b),
	...
	addhlp(lists.c.add(0, lists.sz.y + UI.scale(5)), UI.scale(5), lists.sz.x,
	      lists.new TabButton(0, LocalizationManager.tr("skill.tab.skills"), sktab),
	      lists.new TabButton(0, LocalizationManager.tr("skill.tab.credos"), credos),
	      lists.new TabButton(0, LocalizationManager.tr("skill.tab.lore"),   exps));
	pack();
    }
