...
public class PUtils {
	...
	return(mkimg(img, convolveup(img.getRaster(), tsz, filter)));
    }
    ...
    private static BufferedImage mkimg(BufferedImage src, WritableRaster raster) {
	try {
	    return(new BufferedImage(src.getColorModel(), raster, src.isAlphaPremultiplied(), null));
	} catch(IllegalArgumentException e) {
	    BufferedImage out = TexI.mkbuf(Coord.of(raster.getWidth(), raster.getHeight()));
	    out.setData(raster);
	    return(out);
	}
    }
    public static BufferedImage convolve(BufferedImage img, Coord tsz, Convolution filter) {
	return(mkimg(img, convolve(img.getRaster(), tsz, filter)));
    }
