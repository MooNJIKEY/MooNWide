...
public abstract class GLTexture extends GLObject implements BGL.ID {
	...
	/** GL rejects 0-sized textures; clamp so UI/backbuffer edge cases do not abort the frame. */
	private static int gltdim(int v) {
	    return(Math.max(1, v));
	}
	...
	public Tex2D(GLEnvironment env, Texture2D data, FillBuffers.Array[] pixels) {
	    ...
	    env.prepare((GLRender g) -> {
		    ...
		    int gw = gltdim(data.w), gh = gltdim(data.h);
		    if((pixels[0] != null) && (data.w >= 1) && (data.h >= 1))
			gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, ifmt, data.w, data.h, 0, pfmt, pnum, pixels[0].data());
		    else
			gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, ifmt, gw, gh, 0, pfmt, pnum, null);
		    long mem = data.ifmt.size() * ((data.w >= 1 && data.h >= 1) ? (data.w * data.h) : (gw * gh));
		    for(int i = 1; i < pixels.length; i++) {
			if(pixels[i] != null) {
			    Image<?> img = data.image(i);
			    int iw = gltdim(img.w), ih = gltdim(img.h);
			    if((img.w >= 1) && (img.h >= 1))
				gl.glTexImage2D(GL.GL_TEXTURE_2D, i, ifmt, img.w, img.h, 0, pfmt, pnum, pixels[i].data());
			    else
				gl.glTexImage2D(GL.GL_TEXTURE_2D, i, ifmt, iw, ih, 0, pfmt, pnum, null);
			    mem += data.ifmt.size() * ((img.w >= 1 && img.h >= 1) ? (img.w * img.h) : (iw * ih));
			}
		    }
