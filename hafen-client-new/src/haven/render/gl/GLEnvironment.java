...
public abstract class GLEnvironment implements Environment {
    ...
    private static void disposeq(GLRender r) {
	if(r == null)
	    return;
	try {
	    r.dispose();
	} catch(Throwable t) {
	    /* best-effort cleanup after a failed frame */
	}
    }

    public void process(GL gl) {
	GLRender prep;
	ArrayList<GLRender> copy;
	...
	final boolean needPrep = (prep != null) && (prep.gl != null);
	boolean prepDisposed = !needPrep;
	int ci = 0;
	try {
		...
		for(ci = 0; ci < copy.size(); ci++) {
		    GLRender cmd = copy.get(ci);
		    BufferBGL xf = new BufferBGL(16);
	    ...
	    synchronized(drawmon) {
		if(needPrep && !prepDisposed)
		    disposeq(prep);
		for(int j = ci; j < copy.size(); j++)
		    disposeq(copy.get(j));
	    }
